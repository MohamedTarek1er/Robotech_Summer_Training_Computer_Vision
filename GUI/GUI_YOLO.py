...
letters_model = load_model(r"C:\Users\MoDo\Desktop\ESP\MODELS\Big_Model_PreTrained\YOLO\Big_Model_Final.h5")

with open(r"C:\Users\MoDo\Desktop\ESP\MODELS\Big_Model_PreTrained\YOLO\Big_Model_Dict.pkl", "rb") as f:
...
def run_prediction():
    ...
    numbers_inactive_bool = current_mode != "Letters" and not result.multi_hand_landmarks

    if camera_status == "off" or numbers_inactive_bool:
        if 'NONE' not in predictions:
            predictions['NONE'] = 0
        predictions['NONE'] += 1
    elif current_mode == "Letters":
        results = yolo(frame, classes=[0] ,stream=True)
        ...
        for r in results:
            for box in r.boxes:
                ...
                if hand_img.size > 0:
                    ...
                    preds = letters_model.predict(hand_resized)
                    class_id = int(np.argmax(preds))
                    confidence = float(np.max(preds))

                    if confidence > 0.6:
                        predicted_label = letters_dict[class_id]
    ...
    data_url = frame_to_data_url(frame, quality=70)
    if data_url:
        webcam.source = data_url
        webcam.update()

ui.timer(0.005, run_prediction)
